...
class ItemSet(object):
  ...
  def GetMetadata(self, input_zip):
    ...
    i = self.ITEMS.get("system/recovery-from-boot-exodus.p", None)
    if i:
      i.uid, i.gid, i.mode, i.selabel, i.capabilities = 0, 0, 0o644, None, None
    i = self.ITEMS.get("system/etc/install-recovery-exodus.sh", None)
    if i:
...
def HasRecoveryPatch(target_files_zip):
  try:
    target_files_zip.getinfo("SYSTEM/recovery-from-boot-exodus.p")
    return True
  except KeyError:
    return False
...
""" % bcb_dev)
  ...
  common.ZipWriteStr(output_zip, "META-INF/net/exodus-developers/releasekey",
...
""" % bcb_dev)
  ...
  if updating_recovery:
    ...
    if not target_has_recovery_patch:
      ...
      script.DeleteFiles(["/system/recovery-from-boot-exodus.p",
                          "/system/etc/install-recovery-exodus.sh"])
