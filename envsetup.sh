...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^exodus_") ; then
       EXODUS_BUILD=$(echo -n $1 | sed -e 's/^exodus_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $EXODUS_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       EXODUS_BUILD=
    fi
    export EXODUS_BUILD
...
function print_lunch_menu()
{
    ...
    echo
    if [ "z${EXODUS_DEVICES_ONLY}" != "z" ]; then
...
function breakfast()
{
    target=$1
    local variant=$2
    EXODUS_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/exodus/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the Exodus model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the Exodus github
        T=$(gettop)
        pushd $T > /dev/null
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var EXODUS_VERSION)
        ZIPFILE=exodus-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell getprop ro.exodus.device | grep -q "$EXODUS_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $EXODUS_BUILD, run away!"
    fi
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.exodus.device | grep -q "$EXODUS_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $EXODUS_FIXUP_COMMON_OUT ]; then
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/exodus/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
